-- ═══════════════════════════════════════════
-- PENGUIN PLACE — Migratie 013: Locaties uitbreiding
-- Voegt uitgebreide velden toe aan locaties en groepen,
-- plus twee nieuwe tabellen voor openingstijden.
-- ═══════════════════════════════════════════
...
CREATE TYPE public.locatie_type AS ENUM (
  'kdv', 'bso', 'peuterspeelzaal', 'gastouder', 'combinatie'
);

CREATE TYPE public.locatie_status AS ENUM (
  'actief', 'inactief', 'in_opbouw'
);

CREATE TYPE public.groep_status AS ENUM (
  'actief', 'gesloten', 'alleen_wachtlijst'
);

CREATE TYPE public.inspectie_oordeel AS ENUM (
  'goed', 'voldoende', 'onvoldoende'
);

CREATE TYPE public.cao_type AS ENUM (
  'kinderopvang', 'sociaal_werk', 'overig'
);
...
-- m2_per_kind = m2 / max_capaciteit — berekend in applicatielaag, NIET opgeslagen

-- ───────────────────────────────────────────
-- NIEUWE TABEL: locatie_openingstijden
-- ───────────────────────────────────────────

CREATE TABLE public.locatie_openingstijden (
...
-- ───────────────────────────────────────────
-- NIEUWE TABEL: locatie_openingstijden_uitzonderingen
-- ───────────────────────────────────────────

CREATE TABLE public.locatie_openingstijden_uitzonderingen (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  locatie_id    UUID NOT NULL REFERENCES public.locaties(id) ON DELETE CASCADE,
...
-- ───────────────────────────────────────────
-- UITBREIDING RLS OP locaties
-- (002_rls.sql heeft al: "Beheerder schrijft locaties" — FOR ALL, only beheerder)
-- Voeg locatie-scoped modify toe voor vestigingsmanager / regiomanager / directie
-- ───────────────────────────────────────────
...
-- ───────────────────────────────────────────
-- UITBREIDING RLS OP groepen
-- (002_rls.sql heeft al: "Manager en beheerder schrijven groepen" — has_any_role('beheerder', 'vestigingsmanager'))
-- Voeg regiomanager / directie toe
-- ───────────────────────────────────────────
